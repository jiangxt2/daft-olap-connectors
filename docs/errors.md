...
| `DiscoveryError` | Recoverable split metadata or a planning endpoint failed or returned invalid data. |
| `AuthenticationError` | A database endpoint or worker transport rejected credentials. This is never eligible for discovery fallback. |
| `DatabasePermissionError` | A database endpoint or worker transport reported insufficient privilege. This is never eligible for discovery fallback. |
| `DatabaseObjectNotFoundError` | A structured database response confirmed that a requested database object does not exist. This is never eligible for discovery fallback. |
...
Structured driver fields such as ClickHouse code/name, MySQL vendor code or SQLSTATE, and ADBC
status are the primary authentication, permission, and object-not-found signals. Doris can report a
missing object under its generic vendor code 1105; only for that code, the connector checks a closed
allowlist of not-found markers solely to select the public error category. Raw driver text is never
retained, propagated, or logged. `QuerySpec.__repr__` also hides complete SQL and all values.
...
When `discovery_policy="single"` converts a `DiscoveryError` into one ordinary task, the connector
logs one warning containing only the represented database/table names and the public error
category. It does not log driver exception text, SQL, parameter values, credentials, or a
traceback. Explicit `split="single"`, zero limits, and empty tables are normal single-task paths and
do not emit this failure warning.

Authentication, permission, and confirmed object-not-found errors propagate even when
`discovery_policy="single"`. A successful Doris plan containing no tablets is not an error and
produces one `LIMIT 0` task without a fallback warning. If a task-owned Doris close completes after
its original awaiter was cancelled and fails, the asynchronous callback logs only the close
exception category.
